...
def main(v=1,data="LGG",file=1,b=-1):
    if data=="LGG":
    ...
    if b!=-1:
        if v==1:
            s,u,G=glkpca.gLKPCA_v1(X,b)
            kern=X
        elif v==2:
            s,u,G,kern=glkpca.gLKPCA_v2(X,b)
        elif v==3:
            s,u,G,kern=glkpca.gLKPCA_v3(X,b)
        elif v==4:
            s,u,G,kern=glkpca.gLKPCA_v4(X,b)
    ...
    for i in range(11):
        if v==1:
            s,u,G=glkpca.gLKPCA_v1(X,i/10)
            kern=X
        elif v==2:
            s,u,G,kern=glkpca.gLKPCA_v2(X,i/10)
        elif v==3:
            s,u,G,kern=glkpca.gLKPCA_v3(X,i/10)
        elif v==4:
            s,u,G,kern=glkpca.gLKPCA_v4(X,i/10)
...
if __name__=='__main__':
    ...
    args.add_argument('--modality','-m',help="Which modality? (an integer)")
    args.add_argument('--bval','-b',nargs='?',default=-1,help="At what b? (a number)")
    pargs = args.parse_args()
    b = float(pargs.bval)
    sil,f,labels,kern,G=main(2,pargs.dataset,int(pargs.modality),b=b)
    print("b,\tsil,\tf")
